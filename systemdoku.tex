...
Im Rahmen dieses Projektes war ich hauptsächlich für die Mechanische Konstruktion und den Aufbau der Baugruppe verantwortlich. Darunter fällt die CAD gestützte Konstruktion, die Auswahl mechanischer Komponenten, die Fertigung der Werkstücke und die Anpassung der Mechanik im Laufe des Projektes.
...
Des Weiteren wählten wir ein Grunddesign, das die Servo Kurbel in der Nulllage parallel zur Standfläche orientiert ist. Mit einem positiven und negativen Arbeitsraum ergibt sich so eine maximale Änderung der Plattform bei eher kleineren Winkeländerungen des Servowinkels.
...
Bearbeitet wurde dieser Teil von Marek Gerber.
...
funktional als auch mechanisch stabil ist.
...
geeignet für die angestrebte Verwendung waren. Deshalb haben wir uns für die Verwendung eines
externen PWM-Drivers entschieden. Dieser kann bis zu 16 Kanäle mit Hardware-PWM bereitstellen.
...
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.7\textwidth}{\centering\vspace{20mm}
    [Foto: Technikbox im Gehäuse – offen]
  \vspace{20mm}}}
...
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.7\textwidth}{\centering\vspace{20mm}
    [Foto: Technikbox – geschlossen ohne Raspberry Pi]
  \vspace{20mm}}}
...
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.7\textwidth}{\centering\vspace{20mm}
    [Foto: Verkabelung der Technikbox]
  \vspace{20mm}}}
  \caption{Verkabelung der Technikbox}
\end{figure}

\begin{figure}[h]
  \centering
  \fbox{\parbox{0.7\textwidth}{\centering\vspace{20mm}
    [Baugruppenzeichnung: Technik-Stack]
  \vspace{20mm}}}
  \caption{Baugruppenzeichnung Technik-Stack}
\end{figure}
...
dem Winkel $\varphi$, so kann einfach der Normalenvektor $n$ der ebenen Position mit zwei
...
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.7\textwidth}{\centering\vspace{20mm}
    [Ablaufdiagramm: func\_kinematik\_main]
  \vspace{20mm}}}
...
180°, als kann auch der mit ihm verbundene untere Arm sich nur um 180° drehen.
...
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{25mm}
    [Diagramm: Look-Up Erreichbarkeit (Höhe über Achse Servo zu Mittelpunkt)]
  \vspace{25mm}}}
...
Berechnungen für die Plattform-Winkel festgelegt ist, ergibt sich eine Z-Höhe, bei der der der
...
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.6\textwidth}{\centering\vspace{30mm}
    [Schaubild: 2D-System mit Servo, unterem Arm, oberem Arm und Endpunkt Plattform]
  \vspace{30mm}}}
...
\section{Ansteuerung der Servos}

Nachdem im vorherigen Kapitel die Einstellwinkel für die Servos berechnet wurden, müssen
diese nun auch in die entsprechenden Positionen gebracht werden. Dafür muss an die Servos
ein PWM-Signal gesendet werden. Der Raspberry Pi kann jedoch nur zwei PWM-Signale mit
Hardware erzeugen. Zumindest das dritte PWM-Signal müsste softwareseitig modelliert werden.
Tests haben jedoch festgestellt, dass dieses Signal nicht konstant ist und die Servos zu vibrieren
beginnen. Deshalb werden alle drei Servos mit Hilfe eines zusätzlichen Boards per Hardware-PWM betrieben.

In der Funktion \texttt{PCA9685Driver} übernimmt die Ansteuerung der Servos, welche durch Mitarbeit von Leon
Weyand erstellt und optimiert wurde. Dieser Funktion wird der gewünschte Stellwinkel
übergeben, der dann dort zunächst mit einem Modifier multipliziert wird. Dieser Modifier ist ein
Wert, der für jeden Servo bestimmt wurde, um z.\,B. die Drehrichtung anzupassen, oder mit
einem Gain die Drehung bei gegebenem Winkel etwas zu spreizen. Anschließend wird dieser
Wert mit einem Offset addiert. Dieser Offset hilft, dass der Servo bei dem Stellwinkel 0° auch
wirklich senkrecht, in der gewünschten Nullposition steht. Diese Parameter erlauben die
Kalibrierung der Servos.

Anschließend wird über I\textsuperscript{2}C die gewünschte PWM-Frequenz an das Board gesendet. Die Frequenz
verhält sich proportional von der Maximalfrequenz zum Stellwinkel vom Gesamtweg.

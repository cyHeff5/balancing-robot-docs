...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
      node distance=1.9cm,
      box/.style={draw, rounded corners, minimum width=2.8cm, minimum height=1.2cm, align=center, fill=white},
      mem/.style={draw, rounded corners, minimum width=2.8cm, minimum height=1.2cm, align=center, fill=gray!10},
    ...
    \node[mem, right=of detect] (result) {\textbf{Letztes Ergebnis}\\{\small (Lock-gesichert)}};
    \node[box, right=of result] (ctrl) {\textbf{Regelkreis}\\{\small liest bei Bedarf}};

    \draw[arr] (stream) -- node[above]{\small schreibt} (frame);
    \draw[arr] (frame) -- node[above]{\small liest} (detect);
    \draw[arr] (detect) -- node[above]{\small schreibt} (result);

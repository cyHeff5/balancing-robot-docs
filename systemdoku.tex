...
Die Plattform wird von drei Servomotoren über eine Tripod-Mechanik gekippt. Eine Kamera erfasst kontinuierlich das Bild der Plattform; eine Bildverarbeitungskomponente erkennt die aktuelle Ballposition. Ein PID-Regler berechnet daraus die notwendigen Kippwinkel der Plattform, die über die inverse Kinematik in Servowinkel umgerechnet und an die Motoren übertragen werden.
...
\begin{tabular}{ll}
  ...
  \texttt{control}             & Enthält den PID-Regler \\
  \bottomrule
\end{tabular}
\end{center}

Der Einstiegspunkt für den laufenden Betrieb ist \texttt{main.py}.
...
Im laufenden Betrieb läuft \texttt{main.py} in einer festen
Regelschleife mit 120\,Hz. Pro Durchlauf liest der Regler den aktuellen
...
läuft dabei in zwei parallelen Hintergrund-Threads, sodass die Regelschleife nicht
...
Um das System auf dem Laptop zu testen, reicht es in \texttt{main.py}
...
in 16 verschiedene Positionen (flach, vier Achsrichtungen in zwei Winkelstufen,
vier Diagonalen und eine abschließende Flachstellung), und die Kamera macht bei jeder Position automatisch
...
\section{Hintergrund-Threads und Frame-Auswahl}
...
Dieses Problem wird durch zwei parallel laufende Hintergrund-Threads gelöst.
Der \textbf{Stream-Thread} holt in einer Endlosschleife neue Frames von der
Kamera und schreibt immer den neuesten Frame in eine gemeinsame Variable.
Der \textbf{Detection-Thread} wartet auf ein neues Frame, liest es aus dieser
Variable und führt die Ballerkennung durch. Das Ergebnis, eine
\texttt{BallMeasurement}-Instanz mit Position, Radius und Zeitstempel,
wird ebenfalls in einer Lock-gesicherten Variable gespeichert.
Wenn der Regler eine neue Messung braucht, liest er diese Variable
über \texttt{get\_last\_measurement()} aus. Der Aufruf blockiert nie.

\begin{figure}[h]
  \centering
  \begin{tikzpicture}[
      node distance=2.0cm,
      box/.style={draw, rounded corners, minimum width=2.8cm, minimum height=1.1cm, align=center, fill=white},
      mem/.style={draw, rounded corners, minimum width=2.8cm, minimum height=1.1cm, align=center, fill=gray!10},
      arr/.style={-{Stealth}, thick},
    ]
    \node[box] (stream) {\textbf{Stream-Thread}\\{\small holt Frames}};
    \node[mem, right=of stream] (frame) {\textbf{Neuester Frame}\\{\small (Lock-gesichert)}};
    \node[box, right=of frame] (detect) {\textbf{Detection-Thread}\\{\small erkennt Ball}};

    \node[mem, below=1.2cm of detect] (result) {\textbf{Letztes Ergebnis}\\{\small (Lock-gesichert)}};
    \node[box, left=of result] (ctrl) {\textbf{Regelkreis}\\{\small liest bei Bedarf}};

    \draw[arr] (stream) -- node[above]{\small schreibt} (frame);
    \draw[arr] (frame) -- node[above]{\small liest} (detect);
    \draw[arr] (detect) -- node[right]{\small schreibt} (result);
    \draw[arr] (result) -- node[above]{\small liest} (ctrl);

    \draw[arr, dashed] (stream.north) to[out=120,in=60,looseness=2]
      node[above]{\small kontinuierlich} (stream.north west);
  \end{tikzpicture}
  \caption{Stream-Thread und Detection-Thread laufen parallel; der Regelkreis liest das fertige Ergebnis ohne zu warten}
\end{figure}

Da Stream- und Detection-Thread auf dieselben Variablen zugreifen, wird
der Zugriff jeweils über ein eigenes Lock abgesichert: eines für den Frame-Puffer,
eines für das Erkennungsergebnis.
...
Die Rohgeschwindigkeit wird aus zwei aufeinanderfolgenden Messungen berechnet,
indem die Positionsänderung durch die vergangene Zeit geteilt wird:

\[
  v_x^{\text{roh}} = \frac{x_{\text{neu}} - x_{\text{alt}}}{\Delta t}, \qquad
  v_y^{\text{roh}} = \frac{y_{\text{neu}} - y_{\text{alt}}}{\Delta t}
\]

Um Messrauschen zu dämpfen, wird die Rohgeschwindigkeit anschließend mit einem
exponentiellen gleitenden Mittelwert (EMA) gefiltert:

\[
  v_x \leftarrow \alpha \cdot v_x^{\text{roh}} + (1 - \alpha) \cdot v_x, \qquad \alpha = 0{,}3
\]

Da der Regelkreis mit 120\,Hz läuft, die Kamera aber nur etwa 25\,FPS liefert,
fragt der Regler häufiger nach als neue Bilder ankommen. In diesem Fall gibt
\texttt{BallTracker.get()} das zuletzt berechnete Ergebnis erneut zurück,
setzt dabei aber \texttt{is\_new\_frame = False}. Der Regler kann so zwischen
echten neuen Messungen und wiederholten Ausgaben unterscheiden und bei
\texttt{is\_new\_frame = False} eine Positions-Prädiktion anwenden.
...
\begin{tabular}{lll}
  ...
  \texttt{vx}, \texttt{vy}       & px/s  & EMA-gefilterte Geschwindigkeit in X- und Y-Richtung \\
  \texttt{v}                     & px/s  & Betrag der Geschwindigkeit \\
  \texttt{dt}                    & s     & Zeit seit dem letzten Kamera-Frame \\
  \texttt{is\_new\_frame}        & –     & \texttt{True} bei neuem Kamera-Frame, \texttt{False} wenn Regler schneller als Kamera \\
  \texttt{timestamp\_s}          & s     & Zeitstempel der Bildaufnahme \\
...
\textbf{Stufe 3 – Hintergrund-Threads statt Einzelaufnahmen.}
In der finalen Implementierung wird die Kamera nicht mehr per Einzelaufnahme
abgefragt, sondern über die in Abschnitt~\ref{sec:videostream} beschriebenen
zwei Hintergrund-Threads betrieben. Der Stream-Thread liest den Videostream
kontinuierlich, der Detection-Thread führt die Ballerkennung darauf aus.
Dadurch entfällt der Overhead der Kamerainitialisierung bei jedem Bildabruf. Die Ergebnisse sind in
Tabelle~\ref{tab:latenz3} dargestellt. Die Gesamtlatenz sank auf
im Mittel \SI{40{,}52}{\milli\second}, was einer Bildrate von 24{,}68\,FPS
entspricht.
\begin{table}[h]
\centering
\caption{Latenzmessung Stufe 3 – Hintergrund-Thread statt Einzelaufnahmen}
\label{tab:latenz3}
\begin{tabular}{lrrrrrr}
  \toprule
  Metrik & Mittelwert (ms) & Median (ms) & p95 (ms) & p99 (ms) & Min (ms) & Max (ms) \\
  \midrule
  get\_frame & 12{,}05 & 11{,}81 & 20{,}45 & 23{,}33 &  7{,}61 & 26{,}67 \\
  detect     & 28{,}47 & 27{,}87 & 36{,}35 & 41{,}48 & 22{,}80 & 45{,}09 \\
  total      & 40{,}52 & 39{,}10 & 54{,}25 & 60{,}62 & 32{,}24 & 66{,}01 \\
  \midrule
  \multicolumn{2}{l}{Geschätzte FPS (Mittelwert)} & \multicolumn{5}{r}{24{,}68} \\
  \bottomrule
\end{tabular}
\end{table}
...
\label{sec:pid_entwurf}

Um die optimalen Parameter für den Regler analytisch bestimmen zu können, muss die Regelstrecke (Plant) zunächst mathematisch modelliert werden. Eine Kugel, die auf einer geneigten Ebene rollt, erfährt nicht die volle Erdbeschleunigung, da ein Teil der potentiellen Energie in Rotationsenergie übergeht. Für eine Vollkugel mit der Masse $m$, dem Radius $r$ und dem Trägheitsmoment $J = \frac{2}{5} m r^2$ ergibt sich aus dem Drallsatz und dem Newtonschen Aktionsprinzip die lineare Beschleunigung $a$ in Abhängigkeit des Neigungswinkels $\alpha$:
\begin{equation}
...
\end{equation}
Nach der Methode der \textbf{Polvorgabe} (Pole Placement)~\cite{unbehauen2008} wird das Nennerpolynom mit der charakteristischen Gleichung eines idealen Verzögerungsgliedes 2. Ordnung (PT$_2$-Glied) gleichgesetzt:
\begin{equation}
    s^2 + K_{\text{sys}} K_d s + K_{\text{sys}} K_p = s^2 + 2\zeta\omega_n s + \omega_n^2
\end{equation}
Durch Koeffizientenvergleich lassen sich die optimalen Reglerparameter $K_p$ und $K_d$ berechnen:
\begin{equation}
    K_p = \frac{\omega_n^2}{K_{\text{sys}}}, \quad K_d = \frac{2\zeta\omega_n}{K_{\text{sys}}}
\end{equation}

Für ein ballbalancierendes System wird der aperiodische Grenzfall gewählt ($\zeta = 1{,}0$), um ein Überschwingen der Kugel zu verhindern. Die Eigenkreisfrequenz $\omega_n$ (Schnelligkeit des Reglers) wurde anwendungsspezifisch auf $\omega_n \approx 3\,\text{rad/s}$ festgelegt. Mit $g = 9{,}81\,\text{m/s}^2$ ergibt sich $K_{\text{sys}} \approx 7{,}007$. Setzt man diese Werte ein, ergeben sich die Parameter zu:
\begin{equation}
    K_p = \frac{3^2}{7{,}007} \approx 1{,}28 \quad \rightarrow \quad \text{Hardware-skaliert:} \approx 0{,}05
\end{equation}
\begin{equation}
    K_d = \frac{2 \cdot 1{,}0 \cdot 3}{7{,}007} \approx 0{,}85 \quad \rightarrow \quad \text{Hardware-skaliert:} \approx 0{,}033
\end{equation}
(Die Skalierung resultiert aus der gerätespezifischen Umrechnung von Einheiten im Programmcode.)

\textbf{Die Notwendigkeit des I-Anteils:} Da die reale Strecke nicht ideal ist, sondern durch Coulomb'sche Reibung und Haftreibung (Losbrechmoment) gestört wird, bleibt der Ball bei kleinen Positionsfehlern stehen. Der Regler wurde daher empirisch um einen Integrator ($K_i = 0{,}008$) erweitert, der diese konstanten Störgrößen ausregelt.
...
\begin{figure}[htbp]
    ...
    \caption{Vollständiges Blockschaltbild der MATLAB/Simulink-Simulation inklusive Regler, Ball-Physik (Plant) und Totzeitgliedern}
    \label{fig:simulink_modell}
\end{figure}

Die Simulation wurde mit einer festen Schrittweite von $0{,}02$\,s parametriert (entspricht $50$\,Hz). Tests mit geringeren Frequenzen zeigten einen drastischen Stabilitätsverlust. Um den Windup-Effekt des Integrators zu verhindern, wurde eine Limitierung (Clamping) implementiert. Überschreitet das Reglersignal eine Ausgangsbeschleunigung von $\pm 2{,}5$\,m/s$^2$, wird die Integration gestoppt. Zusätzlich wurde eine Systemtotzeit eingefügt.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{sprungantwort_scope.png}
    \caption{Simulierte Sprungantwort des geregelten Systems auf der X- und Y-Achse bei $50$\,Hz Abtastrate}
    \label{fig:sprungantwort}
\end{figure}

Die simulierte Sprungantwort (Abbildung~\ref{fig:sprungantwort}) zeigt ein robustes Einregelverhalten der Kugel auf den Setpoint.

\section{Diskreter Regelloop und Hardware-Implementierung}
\label{sec:regelloop}

Die validierte Reglerstruktur wurde als zeitdiskreter Algorithmus in Python portiert. Der PID-Controller berechnet aus der Positionsabweichung und der aktuellen Geschwindigkeit (als negativer D-Anteil zur Vermeidung des Derivative Kicks) die benötigten Beschleunigungswerte $a_{x,\text{req}}$ und $a_{y,\text{req}}$.

Diese Vorgaben werden hart limitiert und an die Kinematik übergeben, wo sie in den benötigten Normalenvektor der Ebene umgewandelt werden:
\begin{equation}
    \theta_x = -\arcsin\!\left(\frac{a_{y,\text{req}}}{0{,}7g}\right), \quad \theta_y = \arcsin\!\left(\frac{a_{x,\text{req}}}{0{,}7g}\right)
\end{equation}

Um hochfrequente Schwingungen und mechanische Vibrationen der Servomotoren zu dämpfen, werden die berechneten Winkel $\theta_x$ und $\theta_y$ anschließend durch einen zeitdiskreten exponentiellen Tiefpassfilter (Low-Pass Filter, $\alpha = 0{,}06$) geglättet, bevor sie an die Servos gesendet werden.

\section{Praktische Herausforderungen und Optimierungen}
\label{sec:herausforderungen}

Während der Inbetriebnahme des realen Systems traten mehrere physikalische und softwaretechnische Limitierungen auf, die iterativ gelöst werden mussten.

\subsection{Hardware-Totzeiten durch Spannungsversorgung}

Initial zeigten die Servomotoren ein träges Ansprechverhalten mit massiven Totzeiten. Die Ursache lag in der Verkabelung: Die Versorgungsspannung der drei Servomotoren wurde direkt über den PWM-Generator (PCA9685) bezogen. Bei schnellen Stellbewegungen zogen die Motoren zu viel Strom, was zu Spannungseinbrüchen führte. Die Lösung bestand darin, die Versorgungsspannung der Servos direkt an einen leistungsstarken 5V-Spannungsregler anzuschließen. Der PCA9685 liefert nun ausschließlich die PWM-Steuersignale, wodurch die künstliche Totzeit vollständig eliminiert wurde.

\subsection{Limitation der Regelzykluszeit (Threading)}

Der Videostream der Kamera lief bereits von Beginn an in einem eigenen Hintergrund-Thread. In der ersten Software-Iteration wurde die Ballerkennung jedoch noch im Regelloop selbst ausgeführt. Da die Erkennung pro Frame etwa \SI{28}{\milli\second} dauert, limitierte dies den Regler auf die Kameraframerate von ca. 25\,Hz, was für eine stabile Regelung nicht ausreicht.

Die Lösung bestand darin, die Ballerkennung ebenfalls in einen eigenen Hintergrund-Thread auszulagern. Damit laufen nun zwei Threads parallel: ein Stream-Thread, der kontinuierlich Frames von der Kamera holt und immer den neuesten Frame vorhält, sowie ein Detection-Thread, der darauf wartet und die Erkennung durchführt. Der Regelloop liest das fertige Ergebnis nur noch ab und führt selbst keine Bildverarbeitung mehr durch. Er läuft dadurch stabil mit $120$\,Hz. Liegt kein neues Kamerabild vor, prädiziert der Regler die aktuelle Position des Balls anhand der letzten Geschwindigkeitsvektoren.

\subsection{Einfluss der Beleuchtung auf das Vision-System}

Eine zu starke Beleuchtung führte zu Reflexionen auf der Balloberfläche. Der HSV-Farbfilter erkannte den Ball zeitweise nicht, was zu sprunghaften Positionsänderungen im Code führte. Diese Sprünge regten den D-Anteil des Reglers extrem an und verursachten erratische Motorbewegungen. Dies wurde durch eine optische Anpassung der Lichtverhältnisse sowie durch den in Abschnitt~\ref{sec:regelloop} beschriebenen Low-Pass-Filter zuverlässig kompensiert.

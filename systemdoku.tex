...
\section{Hintergrund-Thread und Frame-Auswahl}
...
Dieses Problem wird anders gelöst. Ein dedizierter Hintergrund-Thread holt
in einer Endlosschleife neue Frames von der Kamera und führt direkt im selben
Thread die Ballerkennung durch. Das Ergebnis, eine \texttt{BallMeasurement}-Instanz
mit Position, Radius und Zeitstempel, wird in einer Lock-gesicherten Variable
gespeichert. Wenn der Regler eine neue Messung braucht, liest er diese Variable
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \node[box] (cam) {\textbf{Kamera-Thread}\\{\small holt Frames,}\\{\small erkennt Ball}};
    \node[mem, right=of cam] (buf) {\textbf{Letztes Ergebnis}\\{\small (gemeinsamer Speicher,}\\{\small Lock-gesichert)}};
    \node[box, right=of buf] (ctrl) {\textbf{Regelkreis}\\{\small liest bei Bedarf}};
  ...
  \end{tikzpicture}
  \caption{Kamera-Thread und Regelkreis greifen über einen gemeinsamen,
  Lock-gesicherten Speicherplatz auf das jeweils aktuellste Ergebnis zu}
\end{figure}

Damit Kamera-Thread und Regelkreis nicht gleichzeitig auf denselben

...
\end{equation}
Für ein ballbalancierendes System wird ein aperiodischer Grenzfall gewählt ($\zeta = 1{,}0$), um ein Überschwingen der Kugel über den Sollwert zu verhindern. Die Eigenkreisfrequenz wurde auf $\omega_n \approx 3\,\text{rad/s}$ festgelegt. Mit $K_{\text{sys}} \approx 7{,}007$ ergeben sich die skalierten Reglerparameter zu $K_p \approx 0{,}05$ und $K_d \approx 0{,}033$.

Da die reale Strecke durch Coulombsche Reibung und Haftreibung gestört wird, bleibt der Ball bei kleinen Positionsfehlern stehen. Der Regler wurde daher empirisch um einen Integralanteil ($K_i = 0{,}008$) erweitert, der diese konstanten Störgrößen ausregelt.
...
\end{equation}
Um hochfrequente Schwingungen und mechanische Vibrationen der Servomotoren zu dämpfen, werden die berechneten Winkel $\theta_x$ und $\theta_y$ anschließend durch einen zeitdiskreten exponentiellen Tiefpassfilter ($\alpha = 0{,}06$) geglättet, bevor sie an die Servos gesendet werden.
...
Initial zeigten die Servomotoren ein träges Ansprechverhalten mit massiven Totzeiten. Die Ursache lag in der Verkabelung: Die Versorgungsspannung der drei Servomotoren wurde direkt über den PWM-Generator (PCA9685) bezogen. Bei schnellen Stellbewegungen zogen die Motoren zu viel Strom, was zu Spannungseinbrüchen führte. Die Lösung bestand darin, die Versorgungsspannung der Servos direkt an einen leistungsstarken 5-V-Spannungsregler anzuschließen. Der PCA9685 liefert nun ausschließlich die PWM-Steuersignale, wodurch die künstliche Totzeit vollständig eliminiert wurde.

\subsection{Limitation der Regelzykluszeit}

In der ersten Software-Iteration lief die Bildverarbeitung sequenziell zur Regelschleife. Da die Kamera maximal 30 Frames pro Sekunde liefert, limitierte dies den Regler auf etwa 30\,Hz, was für eine stabile Regelung nicht ausreicht. Die Softwarearchitektur wurde daher auf Multithreading umgestellt. Die Kamera-Schleife läuft nun entkoppelt in einem Hintergrund-Thread, der Regler läuft stabil mit 120\,Hz. Liegt kein neues Kamerabild vor, prädiziert der Regler die aktuelle Ballposition anhand der letzten Geschwindigkeitsvektoren.

\subsection{Einfluss der Beleuchtung auf das Vision-System}

Eine zu starke Beleuchtung führte zu Reflexionen auf der Balloberfläche, durch die der HSV-Farbfilter den Ball zeitweise nicht erkannte. Die daraus resultierenden sprunghaften Positionsänderungen regten den D-Anteil des Reglers stark an und verursachten erratische Motorbewegungen. Dies wurde durch eine Anpassung der Lichtverhältnisse sowie durch den Tiefpassfilter aus Abschnitt~\ref{sec:regelloop} kompensiert.

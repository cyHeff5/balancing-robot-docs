...
Die Module sind in einer klaren Hierarchie angeordnet. Der Regler steht
ganz oben und kommuniziert ausschließlich mit der \texttt{Platform}-Klasse
...
Intern delegiert \texttt{Platform} die Winkelberechnung an die
\texttt{KinematicsEngine} und die Servo-Ansteuerung an den jeweiligen
\texttt{ServoDriver}. Diese beiden Komponenten wissen nichts voneinander.
...
als auch mit \texttt{Platform} kommuniziert.

% Grafik: Schichtendiagramm
\begin{figure}[h]
  \centering
  \fbox{\parbox{0.7\textwidth}{\centering\vspace{15mm}
    [Schichtendiagramm: Vision / Platform $\rightarrow$ Regler $\rightarrow$ Platform
    $\rightarrow$ KinematicsEngine / ServoDriver $\rightarrow$ Hardware]
  \vspace{15mm}}}
...
\texttt{neutral()} und \texttt{close()}. Solange ein Treiber dieses Interface
